    ...
    <div className='bg-gray-100 min-h-screen flex flex-col items-center py-8 gap-12 animate__animated animate__fadeIn'>
      ...
      <form onSubmit={search}>
        <input 
          ...
          required
        />
        <button className='bg-gradient-to-br from-indigo-600 via-blue-500 to-indigo-600 text-white font-bold py-3 px-8 rounded-r-lg hover:scale-105 transition-transform'>Search</button>
      </form>
          ...
          photos.map((item, index)=>(
            <div key={index} className='bg-white rounded-xl bg-whiie'>
              ...
              <div className='p-3'>
                ...
                <a target="_blank" href={item.src.original} className='mt-3 block bg-green-400 font-bold py-2 rounded-lg text-center hover:scale-105 transition-transform duration-300'>
      ...
      {
        photos.length > 0 &&
        <button onClick={loadMore} className='bg-rose-500 py-3 px-16 rounded-lg font-medium text-white hover:scale-110 transition-transform duration-300'>Load more</button>
      }
